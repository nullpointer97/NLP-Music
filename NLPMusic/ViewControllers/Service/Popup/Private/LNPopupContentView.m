	...
	[UIView performWithoutAnimation:^{
		...
		if(buttonStyle == LNPopupCloseButtonStyleRound)
		{
            if (@available(iOS 13.0, *)) {
                ...
                self.popupCloseButton.tintColor = [UIColor blackColor];
            }
		}
		else
		{
...
- (void)_repositionPopupCloseButtonAnimated:(BOOL)animated
{
	...
	if(startingTopConstant != _popupCloseButtonTopConstraint.constant)
	{
		...
		{
			[UIView animateWithDuration:0.3 delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0.0 options:UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionAllowAnimatedContent animations:^{
				[self layoutIfNeeded];
			} completion:nil];
		}
	}
