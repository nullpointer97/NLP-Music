...
class LoginViewController: VKBaseAuthViewController {
    ...
    @IBOutlet weak var tokenAuth: UIButton!
    
    private let keyboardManagerService = KeyboardManagerService()
    private var isInitialLayout: Bool = true
    
    var authType: AuthType = .loginPassword {
        didSet {
            switch authType {
            case .loginPassword:
                loginTextField.placeholder = "Email или номер телефона"
                passwordTextView.placeholder = "Пароль"
                tokenAuth.setTitle("Войти через токен и User Id", for: .normal)
            case .token:
                loginTextField.placeholder = "User ID"
                passwordTextView.placeholder = "Токен"
                tokenAuth.setTitle("Войти через логин и пароль", for: .normal)
            }
        }
    }
    ...
    override func observeError(error: VKError, isNeedCaptchaData: Bool = false) {
        ...
        DispatchQueue.main.async {
            ...
            case .needCaptcha(captchaImg: let captchaImg, captchaSid: let captchaSid):
                guard let login = self.loginTextField.text, let password = self.passwordTextView.text, !login.isEmpty, !password.isEmpty else { return }
                self.asdk_navigationViewController?.pushViewController(CaptchaViewController(captchaUrl: captchaImg, captchaSid: captchaSid, login: login, password: password), animated: true)
    ...
    @IBAction func didTapLogin(_ sender: Any) {
        ...
        case .token:
            VK.sessions.default.login(userId: login.intValue, token: password) {
                self.observeSuccess()
            } onError: { error in
                self.observeError(error: error, isNeedCaptchaData: false)
            }
        }
    }

    @IBAction func didTokenAuth(_ sender: Any) {
        authType = authType == .loginPassword ? .token : .loginPassword
    }
}
