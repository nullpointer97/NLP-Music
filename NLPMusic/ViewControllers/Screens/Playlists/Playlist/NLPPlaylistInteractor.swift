    ...
    func addAudio(audio: AudioPlayerItem) throws {
        ...
        try ApiV2.method(.addAudio, parameters: &parametersAudio, apiVersion: .defaultApiVersion).done { result in
            DispatchQueue.main.async { [self] in
                let audioViewController = (presenter?.vkTabBarController?.viewControllers?.first as? NLPMNavigationController)?.viewControllers.first as? NLPAudioV2ViewController
