...
extension AudioPlayer {
    ...
    public var items: [AudioPlayerItem]? {
        return queue?.queue
    }
    ...
    public func play(item: AudioPlayerItem) {
        play(items: [item])
    }
    ...
    public func play(items: [AudioPlayerItem], startAtIndex index: Int = 0) {
        if !items.isEmpty {
    ...
    public func add(item: AudioPlayerItem) {
        add(items: [item])
    }
    ...
    public func add(items: [AudioPlayerItem]) {
        if let queue = queue {
            queue.add(items: items)
        } else {
            queue = AudioItemQueue(items: items, mode: mode)
            queue?.delegate = self
            if items.indices.contains(0), let realIndex = queue?.queue.firstIndex(of: items[0]) {
                queue?.nextPosition = realIndex
            }
            queue?.add(items: items)
        }
...
extension AudioPlayer: AudioItemQueueDelegate {
    ...
    /// - Returns: A boolean value indicating whether an item should be consider playable in the queue.
    func audioItemQueue(_ queue: AudioItemQueue, shouldConsiderItem item: AudioPlayerItem) -> Bool {
        return secondDelegate?.audioPlayer(self, shouldStartPlaying: item) ?? true
    }
}
