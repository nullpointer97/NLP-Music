    ...
    func handlePlayerEvent(from producer: EventProducer, with event: PlayerEventProducer.PlayerEvent) {
        ...
        case .loadedDuration(let time):
            if let currentItem = currentItem, let time = time.ap_timeIntervalValue {
                ...
                firstDelegate?.audioPlayer(self, didFindDuration: time, for: currentItem)
                secondDelegate?.audioPlayer(self, didFindDuration: time, for: currentItem)
                thirdDelegate?.audioPlayer(self, didFindDuration: time, for: currentItem)
            }

        case .loadedMetadata(let metadata):
            if var currentItem = currentItem, !metadata.isEmpty {
                currentItem.parseMetadata(metadata)
                firstDelegate?.audioPlayer(self, didUpdateEmptyMetadataOn: currentItem, withData: metadata)
                secondDelegate?.audioPlayer(self, didUpdateEmptyMetadataOn: currentItem, withData: metadata)
                thirdDelegate?.audioPlayer(self, didUpdateEmptyMetadataOn: currentItem, withData: metadata)
            }

        case .loadedMoreRange:
            if let currentItem = currentItem, let currentItemLoadedRange = currentItemLoadedRange {
                firstDelegate?.audioPlayer(self, didLoad: currentItemLoadedRange, for: currentItem)
                secondDelegate?.audioPlayer(self, didLoad: currentItemLoadedRange, for: currentItem)
                thirdDelegate?.audioPlayer(self, didLoad: currentItemLoadedRange, for: currentItem)
        ...
        case .progressed(let time):
            if let currentItemProgression = time.ap_timeIntervalValue, let item = player?.currentItem,
                ...
                firstDelegate?.audioPlayer(self, didUpdateProgressionTo: currentItemProgression, percentageRead: percentage)
                secondDelegate?.audioPlayer(self, didUpdateProgressionTo: currentItemProgression, percentageRead: percentage)
                thirdDelegate?.audioPlayer(self, didUpdateProgressionTo: currentItemProgression, percentageRead: percentage)
            }
        ...
        case .startedBuffering:
            ...
            if reachability?.isReachable ?? false || (currentItem?.soundUrl?.ap_isOfflineURL ?? false) {
                state = .buffering
            } else {
