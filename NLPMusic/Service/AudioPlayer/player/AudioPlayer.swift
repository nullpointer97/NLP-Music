...
public class AudioPlayer: NSObject {
    ...
    public internal(set) var currentItem: AudioPlayerItem? {
        didSet {
...
//                decryptFile(info)
                ...
                let playerItem = AVPlayerItem(url: info)
...
//                }
                ...
                if oldValue == nil || oldValue != currentItem {
                    ...
                    secondDelegate?.audioPlayer(self, willStartPlaying: currentItem)
                    thirdDelegate?.audioPlayer(self, willStartPlaying: currentItem)
                }
                
                if let oldValue = oldValue {
                    firstDelegate?.audioPlayer(self, willStopPlaying: oldValue)
    ...
    public weak var firstDelegate: AudioPlayerDelegate?
    public weak var secondDelegate: AudioPlayerDelegate?
    public weak var thirdDelegate: AudioPlayerDelegate?
    ...
    public internal(set) var state = AudioPlayerState.stopped {
        didSet {
            ...
            if state != oldValue {
                ...
                firstDelegate?.audioPlayer(self, didChangeStateFrom: oldValue, to: state)
                secondDelegate?.audioPlayer(self, didChangeStateFrom: oldValue, to: state)
                thirdDelegate?.audioPlayer(self, didChangeStateFrom: oldValue, to: state)
            }
        }
    }
    ...
    func updateNowPlayingInfoCenter() {
        if let item = currentItem {
            MPNowPlayingInfoCenter.default().ap_update(with: item, duration: currentItemDuration, progression: currentItemProgression, playbackRate: player?.rate ?? 0)
        } else {
    ...
    func setupRemoteTransportControls() {
        ...
        commandCenter.changePlaybackPositionCommand.addTarget { [unowned self] remoteEvent in
            ...
            if let event = remoteEvent as? MPChangePlaybackPositionCommandEvent {
                self.seek(to: event.positionTime) { success in
                    if success {
