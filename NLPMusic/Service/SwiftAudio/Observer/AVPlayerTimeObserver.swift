...
protocol NLPAVPlayerTimeObserverDelegate: AnyObject {
    func audioDidStart()
...
class NLPAVPlayerTimeObserver {
    ...
    weak var delegate: NLPAVPlayerTimeObserverDelegate?
    ...
    func registerForBoundaryTimeEvents() {
        ...
        unregisterForBoundaryTimeEvents()
        let startBoundaryTimes: [NSValue] = [NLPAVPlayerTimeObserver.startBoundaryTime].map({NSValue(time: $0)})
