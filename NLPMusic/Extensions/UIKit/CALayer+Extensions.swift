...
//  Extended Messenger
//
//  Created by Ярослав Стрельников on 17.01.2021.
//
...
extension CALayer {
    ...
    func animateKeyframes(values: [AnyObject], duration: Double, keyPath: String, removeOnCompletion: Bool = true, completion: ((Bool) -> Void)? = nil) {
        let k = Float(UIView.animationDurationFactor())
        var speed: Float = 1.0
        if k != 0 && k != 1 {
            speed = Float(1.0) / k
...
private func makeExtendedSpringAnimation(_ keyPath: String, _ damping: CGFloat) -> CABasicAnimation {
    let springAnimation = CASpringAnimation(keyPath: keyPath)
    springAnimation.mass = 3.0
    springAnimation.stiffness = 1000.0
